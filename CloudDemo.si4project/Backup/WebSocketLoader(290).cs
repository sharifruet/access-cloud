    ...
    public class WebSocketLoader
    {
                    ...
                    switch (cmd)
                    {
                        ...
                        case "sendlog":        
                            ...
                            foreach (var ss in attRecords)
                            {
                             ...
                             strRespone = strRespone + ",\"line1\":\"this is line1\"";
                             strRespone = strRespone + ",\"line2\":\"this is line2\"";
                             strRespone = strRespone + ",\"duration\":5";   //if set 0 ,alway display,if not set ,default is 5
                             strRespone = strRespone + ",\"fontsize1\":22"; //if want to set fontsize ,option is 20 ,22,24,28,32
                             strRespone = strRespone + ",\"fontsize2\":22";
                             int color = 0x00ffffff;  //a-rgb 
                             strRespone = strRespone + ",\"color\":" + color;     
        ...
        public static void getalllog(string sn)
        {
            ...
            if (session == null)
                return;
            string cmdstring;
            cmdstring = "{\"cmd\":\"getalllog\",\"stn\":true,\"from\":\"2021-04-22\",\"to\":\"2021-04-22\"}"; //get all log
        ...
        public static void adduser(string sn, int enrollid, int backupnum, int admin, string username)
        {
            ...
            string usernameutf8 = LogHelper.gb2312_utf8(username);
            string cmdstring;
            cmdstring = "{\"cmd\":\"adduser\",\"enrollid\":" + enrollid + ",\"backupnum\":" + backupnum + ",\"admin\":" + admin + ",\"name\":\"" + usernameutf8 + "\"}";
            session.Send(cmdstring);
        }
        public static void displaymessage(string sn, string line1, string line2, int duration, int fontsize1, int fontsize2, int color)
        {
            WebSocketSession session;
            session = GetSessionByID(sn);
            if (session == null)
                return;
            string cmdstring;
            cmdstring = "{\"cmd\":\"displaymessage\",\"line1\":\"" + line1 + "\""; //max string len is 35
            cmdstring = cmdstring + ",\"line2\":\"" + line2 + "\""; //if need
            cmdstring = cmdstring + ",\"duration\":" + duration; //if need default is 5,if set 0 ,will alway display
            cmdstring = cmdstring + ",\"fontsize1\":" + fontsize1; //if need default is 22 option is :20 22 24 28 32
            cmdstring = cmdstring + ",\"fontsize2\":" + fontsize2; //if need default is 22
            cmdstring = cmdstring + ",\"color\":" + color; //if need,default is RGB 0x00ffffff white.
            cmdstring=cmdstring+"}";
            session.Send(cmdstring);
        }
    }
